...
D3SpriteRenderModel::render(RenderEngine *re) {
    ...
    glBegin(GL_QUADS);
        ...
        glVertex3f(0.f, m_rcDrawArea.l, 0.f);

        //Top-right vertex (corner)
        glTexCoord2f(fTexRight, fTexTop);
        glVertex3f(m_rcDrawArea.w, m_rcDrawArea.l, 0.f);
...
D3SpriteRenderModel::getDrawArea() {
    Point ptPos = getPosition();
    return Rect(ptPos.x + m_rcDrawArea.x, ptPos.y + m_rcDrawArea.y, m_rcDrawArea.w, m_rcDrawArea.l);
}
