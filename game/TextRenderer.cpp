...
int TextRenderer::getPrevLine(const char *str, int start) {
    int i;
    for(i = start; (i >= 0 && str[i] != '\n'); ++i) { }
    if(i < 0) return 0;
    else return i;
}
...
void TextRenderer::render(const char *str, float x, float y, int start, int end, float size) {
    float x_start = x;
