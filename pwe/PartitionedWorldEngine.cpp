...
PartitionedWorldEngine::generateArea(uint uiAreaId) {
    ...
    m_mWorld[uiAreaId].m_pOctree = new WorldOctree(getId(), uiAreaId, Box(-100.f,-100.f,-100.f,200.f,200.f,200.f));
...
//printf(__FILE__" %d %s\n",__LINE__, className.c_str());
    ...
    if(itArea->second.m_pOctree == NULL) {
        itArea->second.m_pOctree = new WorldOctree(getId(), uiAreaId, bxBounds, 2.f);
    }
